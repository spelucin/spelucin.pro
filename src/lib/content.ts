...
const DEFAULT_SITE_SEO = {
  ...
  sameAs: ["https://linkedin.com/in/spelucin", "https://github.com/spelucin"],
...
function mapCaseStudyEntry(slug: string, entry: Record<string, unknown>): CaseStudyWithSeo {
  return {
    ...
    market: entry.market as string,
    timeline: entry.timeline as string,
    role: entry.role as string,
    team: (entry.team as string) || "",
    challenge: entry.challenge as string,
    whatIdid: entry.whatIdid as string[],
    results: entry.results as CaseStudy["results"],
    whyItWorked: entry.whyItWorked as string,
    tools: (entry.tools as string) || "",
    serviceSlug: (entry.serviceSlug as string) || undefined,
    serviceName: (entry.serviceName as string) || undefined,
    seo: {
